...
import com.mrahmed.todo.Activity.EditActivity;
...
//        holder.textDate.setText("Date: " + todo.getDate());
        ...
        holder.deleteButton.setOnClickListener(v -> {
            // Get the Firebase key of the todo item to be deleted
            String firebaseKey = todoList.get(position).getFirebaseKey();

            // Get a reference to the "todos" node in Firebase Realtime Database
            DatabaseReference databaseReference = FirebaseDatabase.getInstance().getReference("todos");

            // Remove the item with the corresponding key
            databaseReference.child(firebaseKey).removeValue()
                    .addOnSuccessListener(aVoid -> {
                        // Item deleted successfully, update RecyclerView
            ...
            intent.putExtra("todo", todo); // Pass the todo item data
